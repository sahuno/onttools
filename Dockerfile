...
RUN add-apt-repository ppa:deadsnakes/ppa && \
    ...
    python3.12 -m pip install --break-system-packages --no-cache-dir --upgrade pip setuptools wheel && \
    rm -rf /var/lib/apt/lists/*
...
# Copy from builder (selective to avoid library conflicts)
...
RUN wget https://cdn.oxfordnanoportal.com/software/analysis/dorado-1.3.1-linux-x64.tar.gz && \
    tar -xzf dorado-1.3.1-linux-x64.tar.gz && \
    mv dorado-1.3.1-linux-x64/bin/* /usr/local/bin/ && \
    mv dorado-1.3.1-linux-x64/lib/* /usr/local/lib/ && \
    rm -rf dorado* && \
    ldconfig

# Install pod5
RUN pip3 install --break-system-packages pod5
...
# Install whatshap
RUN pip3 install --break-system-packages whatshap
...
# Install MultiQC
RUN pip3 install --break-system-packages multiqc

# Install deepTools
RUN pip3 install --break-system-packages deeptools

# Install igv-reports
RUN pip3 install --break-system-packages igv-reports
...
# Clean up - COMMENTED OUT due to libstdc++ conflict with Dorado libraries
# See LIBSTDC_FIX.md for details
# RUN apt-get clean && \
